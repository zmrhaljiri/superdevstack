...
class HtmlWebpackInlineSVGPlugin {
                ...
                compilation.plugin(
                    ...
                    (htmlPluginData, callback) => {
                        ...
                        if (!this.outputPath) {
                            console.log(
                                chalk.red(
                                    'no output path found on compilation.outputOptions'
                                )
                            )
                        ...
                        if (!filename) {
                            console.log(
                                chalk.red(
                                    'no filename found on htmlPluginData.outputName'
                                )
                            )
        ...
        if (!this.runPreEmit) {
            compiler.plugin('after-emit', (compilation, callback) => {
                if (!this.files.length) {
                    console.log(
                        chalk.green('no files passed for SVG inline to process')
                    )
    ...
    updateOutputFile(html, filename) {
        if (!this.outputPath || !filename)
            return Promise.reject(
                new Error(
                    'outputPath & filename must be set to update output file'
                )
            )
    ...
    processOutputHtml(html, inlineImage) {
        return new Promise((resolve, reject) => {
            ...
            fs.readFile(
                path.resolve(this.outputPath, svgSrc),
                'utf8',
                (err, data) => {
                    if (err) reject(err)
                    ...
                    config.plugins = _.map(configObj, (value, key) => ({
                        [key]: value,
                    }))
